...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune a Transformer model for Passage Spoilers Extraction"
    )
    parser.add_argument(
    ...
    if args.model_name:
        config.task2_passage_model_name = args.model_name
    if args.output_dir:
        config.task2_passage_output_dir = args.output_dir
    if args.lr:
        config.task2_lr = args.lr
    if args.epochs:
        config.task2_passage_epochs = args.epochs

    set_seed(config.seed)

    # Load and filter dataset for PASSAGE only
    print("Loading and filtering passage datasets...")
    train_loader = JSONLLoader(args.train_path).filter_by_tag("passage")
    val_loader = JSONLLoader(args.val_path).filter_by_tag("passage")
    ...
    preprocessor = QAPreprocessor(
        model_name=config.task2_passage_model_name,
    ...
    preprocessor.export_stats(config.task2_passage_output_dir)

    model = AutoModelForQuestionAnswering.from_pretrained(
        config.task2_passage_model_name
    )
